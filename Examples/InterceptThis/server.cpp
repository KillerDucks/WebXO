// Includes
#include "../../src/WebXLib/Sockets.hpp"
...
int main(int argc, char* argv[])
{
    ...
    // Setup The Interceptor Settings
    auto iSettings = WebXO::InterceptSettings();

    auto interceptFunc = [=](WebXO::HTTPReq &req) -> CompBuffer 
                {
                    // Make sure we are targetting the correct method [TODO]
                    std::string sHTTPMethod(req.requestType.substr(0, req.requestType.find(' ')));
                    // Null out RequestType to active the InterceptedBuffer
                    req.requestType = "-1";
                    // Now to allow the external handling of the requests + Hand off the buffer
                    return socketServer();
                };

    iSettings.callback = interceptFunc;

    // Init the HTTP class + Set the base path of the HTTP Files
    WebXO::HTTP http(optsCLI.basePath, iSettings);
